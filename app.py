...
ASSET_META_DIR = Path("data/assets_meta")
...
def load_asset_meta(tenant: str) -> dict[str, dict]:
    return load_json_file(ASSET_META_DIR / f"{tenant}.json", {})


def save_asset_meta(tenant: str, payload: dict[str, dict]) -> None:
    save_json_file(ASSET_META_DIR / f"{tenant}.json", payload)


def set_asset_company(tenant: str, asset_id: str, company: str) -> None:
    meta = load_asset_meta(tenant)
    entry = meta.get(asset_id, {})
    entry["company"] = company.strip()
    meta[asset_id] = entry
    save_asset_meta(tenant, meta)


def asset_company(tenant: str, asset_id: str) -> str:
    return str(load_asset_meta(tenant).get(asset_id, {}).get("company", ""))


def load_companies(tenant: str) -> list[str]:
    rows = load_json_file(COMPANIES_DIR / f"{tenant}.json", [])
    return [str(r).strip() for r in rows if str(r).strip()]


def save_companies(tenant: str, companies: list[str]) -> None:
    unique = sorted(set(c.strip() for c in companies if c.strip()))
    save_json_file(COMPANIES_DIR / f"{tenant}.json", unique)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/live-dashboard":
            ...
            d = platform.dashboard()
            meta = load_asset_meta(cur["tenant"])
            due_aug = []
            for item in d.get("due_inspections", []):
                due_aug.append({**item, "company": str(meta.get(item.get("asset_id", ""), {}).get("company", "-") or "-")})
            d["due_inspections"] = due_aug
...
"""
    ...
    def render_ai_sidebar(self, cur: dict, page_name: str = "dashboard") -> str:
        history = cur.get("ai_history", [])[-6:]
        bubbles = "".join(
            f"<div class='bubble user'>{escape(i['q'], quote=True)}</div><div class='bubble ai'>{escape(i['a'], quote=True)}</div>"
            for i in history
        ) or "<p class='muted'>Noch keine Fragen gestellt.</p>"
        pending = cur.get("pending_ai_action", "")
        pending_box = f"<p class='muted'><b>Ausstehend:</b> {escape(pending)}</p>" if pending else ""
        return f"""
<div class='card' style='position:sticky;top:1rem'>
  <h3>AI-Chat</h3>
  {pending_box}
  <div class='chat' style='max-height:320px;overflow:auto'>{bubbles}</div>
  <form method='post' action='/ask-ai'>
    <input type='hidden' name='source_page' value='{escape(page_name)}'>
    <div class='field' style='grid-column:1/-1'>
      <label for='question-inline'>Nachricht</label>
      <textarea id='question-inline' name='question' placeholder='Frage an AI…' required></textarea>
    </div>
...
"""
    ...
    def dashboard_page(self, cur: dict, platform) -> str:
        ...
        for item in d["due_inspections"]:
            company = str(meta.get(item.get("asset_id", ""), {}).get("company", "-") or "-")
            due_entries.append({**item, "company": company})
        due_entries.sort(key=lambda x: (x.get("due_date", ""), x.get("company", ""), x.get("asset", "")))
        due_rows = "".join(
            f"<tr><td>{escape(item['asset'])}</td><td>{escape(item['company'])}</td><td>{escape(item['regulation'])}</td><td>{escape(item['due_date'])}</td><td>{traffic_badge(item['state'])}</td></tr>"
            for item in due_entries
...
<div style='display:grid;grid-template-columns:minmax(0,1fr) 360px;gap:1rem;align-items:start'>
  <div>
    ...
    <div class='card stack'>
      ...
      <div class='grid'>
        <div class='field'><label for='due-sort'>Sortieren</label><select id='due-sort'><option value='due'>Nach Fälligkeit</option><option value='company'>Nach Unternehmen</option></select></div>
        <div class='field'><label>&nbsp;</label><button class='secondary' type='button' id='toggle-due-list'>Toggle Liste</button></div>
      </div>
      <div class='table-wrap' id='due-list-wrap'>
        <table>
  ...
  <div>
    {self.render_ai_sidebar(cur, 'dashboard')}
  </div>
</div>
<script>
async function refreshDashboard() {{
  try {{
    ...
    body.innerHTML = '';
    const sortMode = (document.getElementById('due-sort') || {{value:'due'}}).value;
    const due = [...(data.due_inspections || [])];
    due.sort((a, b) => sortMode === 'company' ? String(a.company||'').localeCompare(String(b.company||'')) : String(a.due_date||'').localeCompare(String(b.due_date||'')));
    for (const item of due) {{
      ...
      row.innerHTML = `<td>${{item.asset}}</td><td>${{item.company || '-'}}</td><td>${{item.regulation}}</td><td>${{item.due_date}}</td><td><span style="display:inline-flex;align-items:center;gap:.35rem"><span style="width:.62rem;height:.62rem;border-radius:999px;background:${{color}};display:inline-block"></span>${{label}}</span></td>`;
...
if (dueSort) dueSort.addEventListener('change', refreshDashboard);
const toggleBtn = document.getElementById('toggle-due-list');
if (toggleBtn) toggleBtn.addEventListener('click', () => {{
  const wrap = document.getElementById('due-list-wrap');
  if (!wrap) return;
...
"""
    ...
    def assets_page(self, cur: dict, platform) -> str:
        ...
        meta = load_asset_meta(cur["tenant"])
        companies = load_companies(cur["tenant"])
        company_options = "".join(f"<option value='{escape(c)}'>{escape(c)}</option>" for c in companies)

        edit_asset = platform.assets.get(edit_asset_id) if edit_asset_id else None
        edit_company = str(meta.get(edit_asset_id, {}).get("company", "")) if edit_asset_id else ""
        ...
        for asset in platform.assets.values():
            ...
            if state == "archived" and not show_archived:
                continue
            company = str(meta.get(asset.asset_id, {}).get("company", "")) or "-"
            rows.append(
...
<div class='card'>
  ...
  {info_box('Was und warum?', 'Lege erst Unternehmen, dann Anlagen und Prüfpläne an. Bearbeiten/Archivieren/Löschen sind direkt möglich.')}
  <div class='grid'>
    <div class='card'>
      <h3>Unternehmen erstellen</h3>
      <form method='post' action='/company-create'>
        {label_input('Unternehmensname', 'company_name', 'z. B. Muster GmbH')}
        <div class='field'><button class='secondary' type='submit'>Unternehmen speichern</button></div>
      </form>
      <p class='muted'>Vorhanden: {', '.join(escape(c) for c in companies) if companies else 'noch keine'}</p>
    </div>
    <div class='card'>
      <h3>Anlage + Plan</h3>
      <form method='post' action='/add-asset-plan'>
        ...
        <div class='field'><label for='company_name'>Unternehmen</label><input list='company-list' id='company_name' name='company_name' placeholder='z. B. Muster GmbH'></div>
...
<div class='card'>
  ...
  <form method='post' action='/asset-update'>
    ...
    <div class='field'><label for='company_name_edit'>Unternehmen</label><input list='company-list' id='company_name_edit' name='company_name' placeholder='z. B. Muster GmbH' value='{escape(edit_company)}'></div>
...
"""
    ...
    def do_POST(self) -> None:
        ...
        try:
            if parsed.path == "/user-create":
            ...
            elif parsed.path == "/company-create":
                if not self.require_capability("plan"):
                    self.send_error(403)
                    return
                company_name = self.safe_form_value(form, "company_name")
                companies = load_companies(cur["tenant"])
                if company_name not in companies:
                    companies.append(company_name)
                    save_companies(cur["tenant"], companies)
                write_audit(cur["tenant"], cur["user_id"], cur["role"], "company_create", {"company": company_name})
                self.redirect_with_message("Unternehmen gespeichert.", page_name="assets")
                return
                ...
                set_asset_company(cur["tenant"], form["asset_id"][0], self.optional_form_value(form, "company_name", ""))
                company_name = self.optional_form_value(form, "company_name", "")
                if company_name:
                    companies = load_companies(cur["tenant"])
                    if company_name not in companies:
                        companies.append(company_name)
                ...
                company_name = self.optional_form_value(form, "company_name", "")
                set_asset_company(cur["tenant"], asset_id, company_name)
                if company_name:
                    companies = load_companies(cur["tenant"])
                    if company_name not in companies:
                        companies.append(company_name)
                ...
                source_page = self.optional_form_value(form, "source_page", "ai")
                self.redirect_with_message("AI-Nachricht verarbeitet.", page_name=source_page)
                return
